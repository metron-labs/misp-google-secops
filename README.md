...
```
┌─────────────┐         ┌──────────────────┐         ┌─────────────────┐
│    MISP     │         │  MISP Forwarder  │         │  Google SecOps  │
│  Platform   │◄────────│   (This Tool)    │────────►│   Entity API    │
│             │  Poll   │                  │ Ingest  │                 │
└─────────────┘         └──────────────────┘         └─────────────────┘
                               │
                               ▼
...
You can reset the synchronization start time to a specific point in the past by updating `HISTORICAL_POLLING_DAYS`.
...
- **Batch Size**: Increase `FORWARDER_BATCH_SIZE` for high-volume environments
